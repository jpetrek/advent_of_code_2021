...
class day<12, 2023> : public day_base<12,2023>
{
    size_t calc(const std::string input, const std::vector<size_t> nums, std::map<std::pair<std::string, std::vector<size_t>>, size_t>& cache)
    {
        ...
        if ((input[0] == '#') || (input[0] == '?'))
        {
            ...
            {
                result += calc(utility::string::slice(input, nums[0] + 1), utility::arrays::slice(nums,1), cache);
            }
        }
    ...
    void run_internal() override
    {
        ...
        {
            auto line = split(input_reader().get_line(), ' ');
            ...
            s1 += calc(line[0], split_convert_vector<size_t>(line[1], ',', stoul()), cache1);
            std::map<std::pair<std::string, std::vector<size_t>>, size_t> cache2;
            s2 += calc(line20, split_convert_vector<size_t>(line21, ',', stoul()), cache2);
        }
        set_star1_result(s1);
        set_star2_result(s2);
    }
